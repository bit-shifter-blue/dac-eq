...
**Key capabilities:**
- Read/write PEQ settings to DSP devices (Tanchjim, Qudelix, etc.)
...
- All 3 servers use Python virtual environments in `mcp/<server>/.direnv/python-3.11/`
...
```bash
# For CLI tool only (dac-eq.py)
pip install -r requirements.txt  # hidapi>=0.14.0

# For MCP servers (if using Claude Code integration)
pip install -r requirements-mcp.txt  # hidapi>=0.14.0, mcp>=1.0.0, scipy>=1.11.0
...
**MCP Servers:**
- Python: 3.11 (managed by direnv in `.direnv/python-3.11/`)
- HID library: hidapi>=0.14.0
- MCP: mcp>=1.0.0
- Optimization: scipy>=1.11.0 (for autoeq-mcp)
